...
* [Error that can occured](#error-that-can-occured)
...
Because of the way macOS (or finder) start executable file, if you simply double-click on the `aquaria_randomizer` file,
the game will not work. This is because the game cannot see the ressources of the Aquaria folder because it does not
search on the correct folder (it search the ressources on the user home folder).
...
check this section: [Error that can occured](#error-that-can-occured)

## Local randomizer via command line

If you want to launch the randomizer using the command line, you will need a command line interpreter (sometime called terminal). You will have to go into the directory of the randomizer with `cd /Users/user/Aquaria_Randomizer/` or some thing like that.
...
If you want to launch the randomizer using the command line, you will need a command line interpreter (sometime called terminal).
You will have to go into the directory of the randomizer with `cd /Users/user/Aquaria_Randomizer/` or some thing like that.
You can open a terminal from Finder by right-clicking (or command+click) on a folder and selecting "New terminal at folder".
