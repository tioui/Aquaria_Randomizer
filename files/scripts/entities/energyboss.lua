	...
	if entity_isState(me, STATE_COLLAPSE) then
		if v.endTextDelay > 0 then
			...
			if v.endTextDelay < 0 then
				...
				debugLog("Creating collectible check")
				local ent = createEntity("CollectibleEnergyBossCheck", "", node_x(collectibleNode), node_y(collectibleNode))
				entity_alpha(ent, 0)
...
function enterState(me)
	...
	elseif entity_isState(me, STATE_COLLAPSED) then
		...
		local collectibleNode = getNode("COLLECTIBLE")
		createEntity("CollectibleEnergyBossCheck", "", node_x(collectibleNode), node_y(collectibleNode))
